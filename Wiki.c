...
//Compute sketches
void compute_sketch(int** sketch, int width, int depth, char** Arr, int* Keys, int no_ele){
    //int** hf = *hashes;
    for(int i = 0; i < no_ele; i++){
        ...
        for(int j = 0; j < depth; j++){
            ...
            //MurmurHash3_x86_32(Arr[i], strlen(Arr[i]),Keys[j], &h);                   //murmur3
            //uint64_t h = fnv_1a(Arr[i],j);                                            //FNV Alg.
            //unsigned int h = ((long)hf[j][0]*hashstr(Arr[i]) + hf[j][1] ) %LONG_PRIME;  //c++ implementation 
            unsigned int h = hashlittle(Arr[i], strlen(Arr[i]), Keys[i]);
            h = h % width;                                                            
            sketch[j][h] += 1;
        }
    }
...
void query_element(int** sketch, int width, int depth,int* Keys, int no_ele, char* q){
  //int** hf = *hashes;
    int min = no_ele;
    for(int iter = 0; iter < depth; iter++){
        ...
        //uint64_t h = fnv_1a(q,iter);
        //h = h%width;
        //unsigned int h = ((long)hf[iter][0]*hashstr(q) + hf[iter][1] ) %LONG_PRIME;  //C++ implementation
        unsigned int h = hashlittle(q,strlen(q), Keys[iter]);
        h = h%width;
...
void normal_CMS(char** Arr1, int no_ele,int width, int depth, uint32_t* Keys){
    ...
    //-------- included only for changing sketch structure ------------
    ...
    for(int i = 0; i < depth; i++){ free(sketch_1[i]); }
    free(sketch_1);
    free(q);
}
